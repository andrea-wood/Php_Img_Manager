...
class ImgManager{
	...
	protected function Init(){
		...
		if (count($_FILES) > 0) {
			...
			foreach ($_FILES as $key => $value) {
				...
				$this->img["orig_name"] = $this->img["name"] . "_orig" . $this->img["type"];
				$this->img["th_name"] = $this->img["name"] . "_th" . $this->img["type"];
				$this->img["md_name"] = $this->img["name"] . "_md" . $this->img["type"];
				...
				if($this->Upload($this->img["size"], $this->img["type"], $uploaded_file, $this->img["rel_orig_path"])){
					...
					echo json_encode(array('response' => true, 'data' => $file));
				} else {
					exit("Code: 2");
				}			
			}
	...
	private function Upload($size, $type, $uploaded_file, $rel_upload_path){		

		if($size <= $this->configs["size_limit_file"]){ // check file size

			if(in_array(substr($type, 1), $this->allowed_ext)){ // check file extension
			
				if (is_uploaded_file($uploaded_file)) {

					if (!move_uploaded_file($uploaded_file, $rel_upload_path)) {
				...
				// set permission
				chmod($rel_upload_path, $this->configs["file_mode"]);

				return true;

			} else {
				exit("not valid file ext");
			}
	...
	public function ResizeImage($resize = array()){
